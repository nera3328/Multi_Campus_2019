# CNN 예시 : 1D convolution을 이용하여 sine 곡선 시계열을 예측해 본다.
# 
...
# 시계열 데이터 (sin)
#data = np.sin(2 * np.pi * 0.03 * np.arange(1001))
#data = np.sin(2 * np.pi * 0.03 * np.arange(1001)) + np.random.random(1001) # noisy sine curve
...
# 아래와 같이 쓰면 편리함
#fc = tf.contrib.layers.fully_connected(inputs=Pl2, num_outputs=nHidden)
#predY = tf.contrib.layers.fully_connected(inputs=fc, num_outputs=1, activation_fn=None)
